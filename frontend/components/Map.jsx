...
import Papa from 'papaparse'; // Import the PapaParse library for parsing CSV
...
export default function Map() {
  ...
  useEffect(() => {
    // Load and parse the just_startups.csv file
    Papa.parse('/data/just_startups.csv', {
      download: true,
      header: true,
      complete: (results) => {
        const startupsWithCoords = results.data
          .map((startup) => ({
            ...startup,
    ...
    // Load and parse the innovation_ecosystem_with_latlong.csv file
    Papa.parse('/data/innovation_ecosystem_with_latlong.csv', {
      download: true,
      header: true,
      complete: (results) => {
        const ecosystemsWithCoords = results.data
          .map((ecosystem) => ({
            ...ecosystem,
